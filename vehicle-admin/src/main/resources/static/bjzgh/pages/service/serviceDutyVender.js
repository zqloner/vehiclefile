...
layui.config({
    ...
    var $ = layui.jquery
        ...
        , pageFlag = true
        , url = "/serviceStationDutyVender/list",
        treeData = [ ];
    // 列表渲染
    var searchObj = {};
    ...
    function tablePlay(data) {
        table.render({
            ...
            cols: [
                [
                    {field: 'venderName', title: '类别', width: '20%'}
                    , {
    ...
    function renderForm() {
        ...
        if (myValue == 1) {
            $("[name='typeName']").html("责任厂家");
        }else if (myValue == 2) {
            $("[name='typeName']").html("故障原因");
        }
        form.render();
    }
                ...
                if (res.code == "200") {
                    ...
                    if (oldList != undefined && oldList.length > 0) {
                        for (var i = 0; i < oldList.length; i++) {
                            ...
                            param.venderName = oldList[i].venderName;
                            myList.push(param)
                        }
                    }
    ...
    function toUpdate(data) {
        $.ajax({
            url: "/serviceStationDutyVender/toUpdate",
            type: "POST",
            data: {"id":data},
            success: function (res) {
                if (res.code == 200) {
        ...
        if (obj.event === 'edit') { // 操作—编辑
            ...
            projectile.elastic({title: " ", content: $("#popup"), area: ['800px', '486px']}, function () {
                ...
                form.on('submit(submitData)', function (data) {
                    var params = new Object();
                    params.venderName = data.field.value;
                    params.id = data.field.id;
                    $.ajax({
                        url: "serviceStationDutyVender/addOrUpdate",
                        type: "POST",
                        data: params,
                        success: function (res) {
                            if (res.code == 200) {
            ...
            layer.confirm('确定要删除该类别吗？', function (index) {
                $.ajax({
                    url: "/serviceStationDutyVender/delete",
                    type: "GET",
                    data: {"id": obj.data.id},
                    success: function (res) {
                        if (res.code == 200) {
