...
layui.config({
        ...
        table.render({
            ...
            cols: [
                [
                    ...
                    {field: 'costOfPrice', title: '单价', width: '25%'},
                ...
                if (res.code == "200") {
                    ...
                    if (oldList != undefined && oldList.length > 0) {
                        for (var i = 0; i < oldList.length; i++) {
                            ...
                            param.projectName = oldList[i].projectName;
                            param.costOfPrice = oldList[i].costOfPrice;
                            param.unit = oldList[i].unit;
                            myList.push(param)
                        }
                    }
    ...
    function toUpdate(data) {
        $.ajax({
            url: "/fixedRepaidProject/toUpdate",
            type: "POST",
            data: {"id":data},
            success: function (res) {
                if (res.code == 200) {
                    $("[name=projectName]").val(res.data.projectName);
                    $("[name=costOfPrice]").val(res.data.costOfPrice);
        ...
        if (obj.event === 'edit') { // 操作—编辑
            ...
            projectile.elastic({title: " ", content: $("#popup"), area: ['800px', '486px']}, function () {
                ...
                form.on('submit(submitData)', function (data) {
                    ...
                    params.costOfPrice = data.field.costOfPrice;
                    params.unit = data.field.unit;
                    params.id = data.field.id;
                    $.ajax({
                        url: "fixedRepaidProject/addOrUpdate",
                        type: "POST",
                        data: params,
                        success: function (res) {
                            if (res.code == 200) {
